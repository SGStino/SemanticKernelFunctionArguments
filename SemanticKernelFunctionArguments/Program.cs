...
async Task RunTest(KernelFunction[] functions, Dictionary<string, List<string>> todos)
{
    ...
    history.AddUserMessage("Can you add 5 common items to my shopping todo list? Make sure you don't have duplicates, if there are any, tell me what they are.");
...
await RunTest([
        KernelFunctionFactory.CreateFromMethod(todos1.add_todos, new(){
            FunctionName = "add_todos",
            Description = "Add todo items to a specific scope/group"
        }),
        KernelFunctionFactory.CreateFromMethod(todos1.remove_todos, new(){
            FunctionName = "remove_todos",
            Description = "Remove todo items from a specific scope/group"
        }),
        KernelFunctionFactory.CreateFromMethod(todos1.get_todos, new(){
            FunctionName = "get_todos",
            Description = "List all todo items of a specific scope/group"
        }),
        KernelFunctionFactory.CreateFromMethod(todos1.get_scopes, new(){
...
await RunTest([
        AIFunctionFactory.Create(todos2.add_todos, new(){
            Name = "add_todos",
            Description = "Add todo items to a specific scope/group"
        }).AsKernelFunction(),
        AIFunctionFactory.Create(todos2.remove_todos, new(){
            Name = "remove_todos",
            Description = "Remove todo items from a specific scope/group"
        }).AsKernelFunction(),
        AIFunctionFactory.Create(todos2.get_todos, new(){
            Name = "get_todos",
            Description = "List all todo items of a specific scope/group"
        }).AsKernelFunction(),
        AIFunctionFactory.Create(todos2.get_scopes, new(){
            Name = "get_Scopes",
            Description = "List all known scopes/groups"
        }).AsKernelFunction()
    ], todos2.todos);
